...
const ChatSidebar = ({ user, stats }) => {
    ...
    return (
        <div className="w-16 md:w-64 border-r border-white/5 bg-slate-900/30 flex flex-col items-center md:items-start p-4 space-y-8 z-20">
            {/* User Profile */}
            <div className="w-full flex items-center gap-3 px-2 mb-4">
                ...
                <div className="hidden md:block overflow-hidden">
                    <h3 className="text-sm font-bold text-white truncate">{user?.name || 'User'}</h3>
            ...
            <div className="w-full space-y-3">
                {navItems.map((item, i) => (
                    <motion.div
                        key={i}
                        whileHover={{ x: 5 }}
                        className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/5 hover:border-white/10 transition-all cursor-default group"
                    >
            ...
            <div className="mt-auto w-full pt-10 hidden md:block">
                <div className="p-4 rounded-2xl bg-gradient-to-br from-emerald-500/10 to-primary/10 border border-white/5">
            ...
            <div className="w-full hidden md:block">
                <div className="glass-dark rounded-2xl p-4 border border-white/10 ring-1 ring-white/5">
                    <div className="flex items-center gap-3 mb-3">
                        <div className="p-2 bg-yellow-400/20 rounded-lg">
                            <Activity size={16} className="text-yellow-400 animate-pulse" />
                        </div>
                        <span className="text-[10px] font-bold text-slate-300">LIVE ANALYSIS</span>
                    </div>
